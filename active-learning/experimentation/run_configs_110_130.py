#!/usr/bin/env python3
"""
Run Multiple Unregularized Logistic Regression Active Learning Configurations (Configs 110-130)

This script runs 21 different unregularized logistic regression configurations 
(configs 110-130) on standardized data. Each configuration is designed based on
insights from our analysis of why config23 succeeded, but now using Logistic
Regression WITHOUT regularization to test if that was the limiting factor.
...
Unregularized Logistic Regression Considerations:
- No regularization (C=1.0, default parameters)
- May overfit on small datasets
- Could capture more complex patterns
- Tests if regularization was actually helping or hurting
- Uses 'lbfgs' solver for better convergence without regularization
"""
...
def create_config_file(config_number, config_dict):
    ...
    with open('simple_active_learning-noreg.py', 'r', encoding='utf-8') as f:
        content = f.read()
...
"""
    ...
    if start_idx == -1:
        print(f"❌ Error: Could not find EXPERIMENT_CONFIG in simple_active_learning-noreg.py")
        return False
    
    # Find the end of the config section
    end_idx = content.find(end_marker, start_idx)
    if end_idx == -1:
        print(f"❌ Error: Could not find CONFIG_NAME in simple_active_learning-noreg.py")
        return False
    ...
    with open('simple_active_learning-noreg.py', 'w', encoding='utf-8') as f:
        f.write(new_content)
...
def run_config(config_number, config_dict):
    ...
    print(f"\n{'='*80}")
    print(f"🚀 RUNNING UNREGULARIZED LOGISTIC REGRESSION CONFIG {config_number}")
    print(f"{'='*80}")
    ...
    print(f"Model: Logistic Regression WITHOUT Regularization (DEFAULT parameters)")
    print(f"🔧 No regularization: C=1.0, penalty='l2' (but C=1.0 means no penalty)")
    print(f"{'='*80}")
    ...
    try:
        ...
        result = subprocess.run([sys.executable, 'simple_active_learning-noreg.py'], 
        ...
        if result.returncode == 0:
            print(f"✅ Unregularized LR Config {config_number} completed successfully in {duration:.1f} seconds")
            ...
            print(f"❌ Unregularized LR Config {config_number} failed with return code {result.returncode}")
            print(f"Error output: {result.stderr}")
            return False
            
    except subprocess.TimeoutExpired:
        print(f"⏰ Unregularized LR Config {config_number} timed out after 30 minutes")
        return False
    except Exception as e:
        print(f"❌ Unregularized LR Config {config_number} failed with error: {e}")
        return False

def main():
    """Main function to run all unregularized logistic regression configurations"""
    
    print("🎯 UNREGULARIZED LOGISTIC REGRESSION ACTIVE LEARNING CONFIGURATION OPTIMIZATION")
    print("="*80)
    print("Running 19 unique unregularized LR configurations: configs 110,111,112,114,115,117,118,119,120,121,122,123,124,125,126,127,128,129,130")
    print("NOTE: Configs 113,116 are duplicates and have been commented out")
    print("Based on analysis: Late iterations (9-11) are critical for performance")
    print("Model: Logistic Regression WITHOUT Regularization (DEFAULT parameters)")
    print("Baseline: Config23 (Regularized LR) - F1: 0.5284 ± 0.0072")
    print("🎯 Goal: Test if lack of regularization was the limiting factor")
    print("="*80)
    
    # Define all unregularized logistic regression configurations
    configs = {
        110: {
            'initial_samples': 300,
        ...
        # 113: DUPLICATE OF CONFIG 23 - COMMENTED OUT TO AVOID CONFUSION
        # {
        #     'initial_samples': 300,
        #     'initial_strategy': 'random',
        #     'batch_size': 68,
        ...
        # 116: DUPLICATE OF CONFIG 56 - COMMENTED OUT TO AVOID CONFUSION
        # {
        #     'initial_samples': 300,
        #     'initial_strategy': 'random',
        #     'batch_size': 68,
    ...
    for config_num in range(110, 131):  # 110 to 130 inclusive
    ...
    # Summary
    print(f"\n{'='*80}")
    print("🎯 UNREGULARIZED LOGISTIC REGRESSION CONFIGURATION RUN SUMMARY")
    print(f"{'='*80}")
    ...
    print(f"📊 Baseline: Config23 (Regularized LR) - F1: 0.5284 ± 0.0072")
    print(f"🎯 Look for unregularized LR F1 scores higher than 0.5284")
    print(f"🔧 All configs use global feature standardization")
    print(f"📊 Model: Logistic Regression WITHOUT Regularization (DEFAULT parameters)")
    print(f"🔧 No regularization: C=1.0, penalty='l2' (but C=1.0 means no penalty)")

    print(f"\n{'='*80}")
    print("🚀 UNREGULARIZED LOGISTIC REGRESSION CONFIGURATION OPTIMIZATION COMPLETE!")
    print(f"{'='*80}")
