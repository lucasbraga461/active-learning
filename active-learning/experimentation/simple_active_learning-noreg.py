#!/usr/bin/env python3
"""
Simple Active Learning Script - Logistic Regression WITHOUT Regularization

This is a simplified version that's easy to understand and use.
Just run this file and it will compare active vs passive learning using
Logistic Regression with DEFAULT parameters (no regularization).

Key Features:
- Uses Logistic Regression with DEFAULT parameters (no regularization)
- Implements the best performing AL strategy (config23)
- Global feature standardization for reproducibility
- Enhanced data preprocessing with sorted column processing
- No regularization to test if that was the limiting factor
"""
...
def qbc_sampling(X_unlabeled, n_samples, X_labeled, y_labeled):
    ...
    # Train a committee of models with NO regularization for Logistic Regression
    models = {
        'lr': LogisticRegression(
            # NO regularization - using default parameters
            random_state=42, 
            max_iter=100,  # Default iterations (should be sufficient)
            solver='lbfgs',  # Better solver for unregularized case
...
def train_and_evaluate(X_train, y_train, X_val, y_val):
    ...
    # Use unregularized Logistic Regression (DEFAULT parameters)
    print("  📊 Using Logistic Regression with DEFAULT parameters (no regularization)")
    
    # NO regularization - using default parameters
    model = LogisticRegression(
        # C=1.0 (default) - no regularization
        # penalty='l2' (default) - but C=1.0 means no penalty
        random_state=42, 
        max_iter=100,  # Default iterations (should be sufficient)
        solver='lbfgs',  # Better solver for unregularized case
...
def main():
    ...
    # ===== MODEL CONFIGURATION =====
    # Using Logistic Regression WITHOUT regularization (DEFAULT parameters)
    print("🚀 EXPERIMENT: Logistic Regression WITHOUT Regularization")
    print("📊 Model: LogisticRegression() - DEFAULT parameters")
    print("🔧 No regularization: C=1.0, penalty='l2' (but C=1.0 means no penalty)")
    print("="*80)
    ...
    EXPERIMENT_CONFIG = {
        ...
        'iteration_strategies': ['uncertainty', 'uncertainty', 'uncertainty', 'uncertainty', 'diversity', 'uncertainty', 'uncertainty', 'diversity', 'uncertainty', 'uncertainty', 'qbc']
    }
    CONFIG_NAME = "config130"
    ...
    print("="*80)
    print("🚀 ENHANCEMENT: Using Logistic Regression WITHOUT Regularization")
    print("🎯 GOAL: Test if lack of regularization was the limiting factor")
    print("🔧 Model: LogisticRegression() with DEFAULT parameters")
    print("="*80)
    ...
    print(f"  Model Type: Logistic Regression (NO regularization)")
    print(f"  Parameters: DEFAULT (C=1.0, penalty='l2', but C=1.0 means no penalty)")
    print()
