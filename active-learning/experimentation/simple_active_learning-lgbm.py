#!/usr/bin/env python3
"""
Simple Active Learning Script with LightGBM

This is a simplified version that's easy to understand and use.
Just run this file and it will compare active vs passive learning using LightGBM.

Key Features:
- Uses LightGBM as the primary model (better for structured data)
- Implements the best performing AL strategy (config23)
- Falls back to Logistic Regression if LightGBM is not available
...
def clean_bank_dataset(df, use_numerical_features=False):
    ...
    # 6. Global feature standardization (important for LightGBM and Logistic Regression)
...
def qbc_sampling(X_unlabeled, n_samples, X_labeled, y_labeled):
    ...
    try:
        ...
        models['lgb'] = lgb.LGBMClassifier(
            n_estimators=50,           # Fewer trees for committee
            max_depth=4,                # Shallow trees for diversity
            learning_rate=0.15,         # Slightly faster learning
            subsample=0.7,              # More aggressive subsampling
            colsample_bytree=0.7,       # More aggressive feature sampling
            min_child_samples=15,       # Require fewer samples per leaf
            random_state=42,
    ...
    models['nb'] = GaussianNB()    # Naive Bayes - very robust for small datasets
...
def train_and_evaluate(X_train, y_train, X_val, y_val):
    ...
    # Option 1: LightGBM (current - best for structured data)
    # Option 2: Regularized Logistic Regression (fallback)
    # Option 3: Naive Bayes (very robust for small datasets)
    ...
    if MODEL_TYPE == 'lightgbm':
        try:
            import lightgbm as lgb
            # LightGBM with careful hyperparameters for small datasets
            model = lgb.LGBMClassifier(
                n_estimators=100,           # Moderate number of trees
                max_depth=6,                # Limit depth to prevent overfitting
                learning_rate=0.1,          # Slow learning rate for stability
            ...
            model = LogisticRegression(
                C=0.1, random_state=42, max_iter=500,
...
def main():
    ...
    MODEL_TYPE = 'lightgbm'  # Options: 'lightgbm', 'logistic', 'naive_bayes'
    ...
    EXPERIMENT_CONFIG = {
        ...
        'iteration_strategies': ['uncertainty', 'uncertainty', 'diversity', 'uncertainty', 'diversity', 'uncertainty', 'uncertainty', 'diversity', 'uncertainty', 'uncertainty', 'qbc']
    }
    CONFIG_NAME = "config100"
    ...
    print("="*80)
    print("🚀 ENHANCEMENT: Using LightGBM instead of Logistic Regression")
    print("🎯 GOAL: Outperform config23's F1 score (current best strategy)")
    print("="*80)
